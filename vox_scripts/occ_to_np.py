...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Convert occupancy grids to numpy alpha arrays.')
    parser.add_argument('input', type=str, help='The input HDF5 file.')
    parser.add_argument('output', type=str, help='The output directory for NPY files.')
    ...
    for n in range(occupancy.shape[0]):
        out_file = os.path.join(args.output, '%d.npy' % n)
        np.save(out_file, occupancy[n].astype(np.float), allow_pickle=False, fix_imports=False)
